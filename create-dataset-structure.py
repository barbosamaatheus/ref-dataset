    ...
    with open(csv_file, newline='', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            with open(left_right_file, "w", encoding="utf-8") as lr_file:
                # 6.1 For each value in "left modifications": write class_name, sink, value
                for value in left_modifications:
                    lr_file.write(f"{class_name},sink,{value}\n")
                # 6.2 For each value in "right modifications": write class_name, source, value
            ...
            with open(right_left_file, "w", encoding="utf-8") as rl_file:
                # 7.1 For each value in "left modifications": write class_name, source, value
                for value in left_modifications:
                    rl_file.write(f"{class_name},source,{value}\n")
                # 7.2 For each value in "right modifications": write class_name, sink, value
                for value in right_modifications:
...
def main():
    # OBS4: Remove all "changed-methods" folders before starting the process
